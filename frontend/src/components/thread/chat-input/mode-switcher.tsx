...
export default function ModeSwitcher({ activeMode, onModeChange }) {
  ...
  return (
    <div
      className="flex flex-row gap-2 items-center rounded-[20px] w-fit border border-white/45 backdrop-blur-[12px] transition-all relative overflow-hidden"
      style={{
        background: '#F7F7F703',
    ...
    >
      {/* Glass shine overlay */}
      <div className="relative flex items-center gap-2 rounded-[20px] w-fit backdrop-blur-[12px] overflow-hidden">
        ...
        <div
          ...
          style={{
            padding: '0.2px',
            background: `
        linear-gradient(to right, #FFFFFF73, #2D2C2C, #FFFFFF73)
      `,
        ...
        <div className="relative z-10 flex flex-row gap-2 items-center">
          {MODES.map((mode, idx) => (
            <button
              ...
              className={
                selected === mode.key
                  ? 'w-10 h-10 rounded-full flex items-center justify-center bg-gradient-to-br from-[#FF522A] to-[#5E5EDE] shadow-lg transition-all duration-150'
